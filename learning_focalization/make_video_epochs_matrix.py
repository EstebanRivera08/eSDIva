...
# loss_vect = data["loss"]
loss_vect = target_loss + energy_loss
...
# loss_vect = np.concatenate(([loss0], loss_vect), axis=0)
# apodization_vect = np.concatenate(([apodization0], apodization_vect), axis=0)
...
epochs = len(loss_vect)


def plot_figure(epoch):
    ...
    XZ_plane = XZ_plane.squeeze().detach().cpu().numpy()
    XY_plane = XY_plane.squeeze().detach().cpu().numpy()
    YZ_plane = YZ_plane.squeeze().detach().cpu().numpy()
    ...
    im_apo = ax_apo.imshow(
        apodization_vect[epoch].reshape((n_elem_y, n_elem_x)),
        cmap="cool",
        vmin=0,
        vmax=1,
    )
    ...
    im_del = ax_del.imshow(
        delays_vect[epoch].reshape((n_elem_y, n_elem_x)),
        cmap="hsv",
        vmin=-period,
        vmax=period,
    )
    ...
    im_targ = ax_targ.imshow(target_matrix.T, cmap="gray", vmin=0, vmax=1)
