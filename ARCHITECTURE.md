# eSDIva Architecture & Internals

Deep reference for understanding eSDIva's internals — for contributors and anyone
...
`Emission` is the primary simulation class. `eSDIva` is a deprecated alias
...
```python
from esdiva.plotting import plot2D_transient_slices
...
- `FieldIITransducer` (`src/esdiva/transducers/fieldii_compat.py`) — transducer built from raw patches
- `from_fieldii_xdc_data(data)` — parse MATLAB `xdc_get(Th,'all')` struct → `FieldIITransducer`
- `from_fieldii_rect_data(rect)` — 26-row `xdc_get(Th,'rect')` matrix → `FieldIITransducer` (corners re-ordered to eSDIva's u/v quad convention from geometry alone, robust to Field II's perimeter corner ordering)
- `from_fieldii_patch_arrays(centres, u, v, hw, hh)` — explicit patch arrays → `FieldIITransducer`
- Treats each Field II mathematical element as one eSDIva element (n_elements = N_patches)
...
- Plane metadata & parsing: `src/esdiva/plotting/plane_utils.py` (`PLANE_META`, `PlaneSpec`, `parse_planes`)
- 2D/Matplotlib: Add to `src/esdiva/plotting/plotting2D.py`
- 3D/PyVista helpers: Add to `src/esdiva/plotting/plotting_pyvista.py`
- 3D/PyVista high-level: Add to `src/esdiva/plotting/plotting3D.py`
- Export new functions from `src/esdiva/plotting/__init__.py`

**Save/Export Convention** (defined in `src/esdiva/plotting/export_utils.py`):
