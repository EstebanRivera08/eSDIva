# PyField Agent Guidelines

## Project Status

This is close to a version to be released. Be careful with the core engine for
computation of acoustic fields `h_sir`. The transducers and utilities are modules in
constant development so feel free to suggest breaking API changes when they improve the
design and readibilty for ease the adoption of the project.
...
11. When performing complicated math, explain briefly the operations performed to allow
    understandability.
...
- Use mkdocs-style links for cross-references to other functions/classes:
  `[function_name][pyfield.module.path.function_name]` or
  `[ClassName][pyfield.module.path.ClassName]`
...
## Release Process (still not released, but keep this in mind for the future)
