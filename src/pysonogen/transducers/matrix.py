...
class MatrixArrayTransducer:
    ...
    def __init__(
        ...
        *,
        ...
        print(f"MatrixArrayTransducer initialized with {self.n_elements} elements and {len(self.sub_quad_verts)} patches.")
        end_time = TIME()
        print(f"Transducer initialized in {end_time - start_time:.4f} seconds.")


    def compute_apodization(self, focus_mm, *, F_over_D = None, apodization_type='rect', plot=False, inline = True):
        """
        ...
        if apodization_type is None:
           ...
           # require ratio_F_over_D property
            ...
            if apodization_type == 'rect':
                ...
                raise ValueError("Invalid apodization type. Must be one of: 'none', 'rect', 'circular', 'hanning', 'hamming'.")
    ...
    def compute_delays(self, *, focus_mm, c = None, plot=False, inline = True):
        ...
        if plot:
            import matplotlib.pyplot as plt
            plt.figure()
            plt.plot(np.arange(self.n_elements), delays*1e6, 'k-', marker='o', markerfacecolor='r', ms = 3)
            plt.title(f"Focusing at: [{x_foc*1e3:.3f} mm, {y_foc*1e3:.3f} mm, {z_foc*1e3:.3f} mm]")
            plt.xlabel("Element #")
