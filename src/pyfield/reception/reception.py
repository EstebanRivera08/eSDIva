...
* ``fst`` / ``sdi`` / ``auto`` (conventional family) — sample each one-way SIR (place its
  corner deltas, double-integrate by cumulative sum) and FFT-convolve them. The convolution
...
the conventional form and Field II.
...
class Reception(ReceptionBase):
    ...
    * ``"spectral"`` (default) — multiply the two closed-form one-way SIR spectra on
