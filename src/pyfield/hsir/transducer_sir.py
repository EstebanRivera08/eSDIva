...
def _fully_sampled_trapezoid(
    ...
    """Helper to fill h_out with trapezoid values for naive method."""
...
def _place_sir_sdi_deltas(d2h, p, idxs, vals, t0, fs, t1, t2, t3, t4, slope):
    """Helper to apply SDI events to d2h and integrate to h_out."""
    evt = 0
    # t1 (+)
    k1f = (t1 - t0) * fs + 1
    k4f = (t4 - t0) * fs + 1
...
def _compute_sir_parallel_points(
    P,
    M,
    T,
    points,
    ...
    patch_tangents : ndarray, shape (M, 6)
    ...
    for p in prange(P):  # ty: ignore[not-iterable]
        ...
        for m in range(M):
            ...
            xp = (
                dx * patch_tangents[m, 0]
                + dy * patch_tangents[m, 1]
                + dz * patch_tangents[m, 2]
            ) * inv_dist
            yp = (
                dx * patch_tangents[m, 3]
                + dy * patch_tangents[m, 4]
                + dz * patch_tangents[m, 5]
            ) * inv_dist
...
def compute_h_sir(
    ...
    patch_tangents = np.ascontiguousarray(np.concatenate([eu_k, ev_k], axis=1))
    h_out, range_k_matrix, min_time, max_time = _compute_sir_parallel_points(
        P,
        M,
        T,
        points,
