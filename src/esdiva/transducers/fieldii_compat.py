"""Import Field II transducer geometry into eSDIva.

Converts the output of MATLAB ``xdc_get(Th, 'all')`` to a
:class:`FieldIITransducer`, which is a standard eSDIva transducer whose
...
Python import::
    ...
    import scipy.io
    from esdiva.transducers import from_fieldii_xdc_data
...
matching eSDIva's quad-vertex ordering (edge v[1]-v[0] = u-direction,
edge v[3]-v[0] = v-direction).

.. note::
   Scale convention: eSDIva uses ``rho / (2 * c²)`` while Field II uses
...
class FieldIITransducer(TransducerBase):
    ...
    The transducer behaves identically to any other eSDIva transducer —
    it can be passed to :class:`~esdiva.emission.Emission`,
    :class:`~esdiva.reception.Reception`, or
    :class:`~esdiva.reception.ReceptionConventional`.
...
def from_fieldii_xdc_data(
    ...
    *,
    ...
    # eSDIva quad ordering: c0 = -u -v, c1 = +u -v, c2 = +u +v, c3 = -u +v
    hw = half_wx[:, None]  # (N, 1)
    hh = half_wy[:, None]  # (N, 1)
...
def from_fieldii_rect_data(
    ...
    *,
    ...
    NOT eSDIva's quad ordering (``c1-c0`` must be the u-tangent and
    ...
    patch_quads = []
    for c4 in corners:
        # Re-order to eSDIva's quad convention from geometry alone: the
...
def from_fieldii_patch_arrays(
    ...
    *,
    ...
    -------
    FieldIITransducer
        An eSDIva transducer with one patch per supplied centre/tangent row.
    """
