...
data.ratio_F_over_D = 1 ; % f/D ratio to compute the active aperture
...
%  ------- transducer characteristics -------
data.tx_N_elem_x = 55 ; % Number of elements in the transducer
data.tx_N_elem_y = 55 ; % Number of elements in the transducer
...
data.tx_N_elem_x, ...       % Number of elements
data.tx_element_width_mm/1000, ...            % Width of elements [m]
data.tx_N_elem_y, ...
data.tx_element_height_mm*ones(1,data.tx_N_elem_y)/1000, ...   % Height of elements [m]
...
% --------- add apodization -----------
% Get apodization profile
[apodization, data] = compute_apodization_matrix(data, apodization_type) ;
data.apodization = apodization ;

apodization_element = reshape(apodization, 1, []) ;
% Set the apodization into the transducer
element_no = (1:data.tx_N_elem_x*data.tx_N_elem_y)';
apo = apodization_element'.*ones(1,data.no_sub_x*data.no_sub_y);
ele_apodization(transducer, element_no, apo)

show_xdc_optimized(transducer)
...
data.M = data.tx_N_elem_x*data.tx_N_elem_y*data.no_sub_x*data.no_sub_y ;
data.P = nx*ny*nz ;
