...
Worked examples that progressively introduce PyField's features — from basic transducer geometry to pulse-echo imaging, brain-atlas integration and STL mesh simulation.
...
Output figures are controlled by `examples/config.py`. Set `FIG_FOLDER` to choose where images are saved and toggle `SAVE_FIG` (or set `PYFIELD_SAVE_FIG=1`) to switch between saving and interactive display.
...
-   :lucide-import: **[17. Import a Field II Probe](example17_import_fieldii_transducer.md)**
    ...
    One-line MATLAB export → `from_fieldii_rect_data` → a native PyField transducer with the original delays and apodization.
