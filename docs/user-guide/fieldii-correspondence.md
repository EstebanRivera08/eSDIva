...
eSDIva deliberately mirrors Field II's conventions so a Field II user can
...
equals `calc_hhp` and eSDIva's RF coincides with both (correlation ≈ 1.0000).

## Time origin (`t0`)

Field II reports absolute time from the excitation start. eSDIva returns each
...
For a lens-focused aperture, eSDIva's time grid is referenced to the
...
eSDIva scales the pulse-echo RF by `ρ₀ / 2c₀²` (the physical scattering
...
Field II is SI everywhere (metres, seconds). eSDIva's user-facing API is
