...
time_start_h = tic ;
[h, start_time] = calc_hp(transducer, data.points);
...
reshaped_pressure = reshape(h,[], size(x_grid,2), size(x_grid,1), size(x_grid,3));  % first two dimensions are space, last is time
...
tN = size(reshaped_pressure, 1);

figure('color', 'white', 'WindowState', 'maximize') ;

for ti = 1:20:tN
    
imagesc(x_vec, z_vec, squeeze(reshaped_pressure(ti, :,round(ny/2),:))')
colormap jet
axis image

xlabel('x, mm')
zlabel('z, mm')
title(['XZ plane: ', num2str(ti),'/', num2str(tN)])
